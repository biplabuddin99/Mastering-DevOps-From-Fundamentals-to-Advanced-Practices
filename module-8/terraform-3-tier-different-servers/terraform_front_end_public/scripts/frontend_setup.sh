...
# Exit on errors — certbot and DNS sections use targeted || true where optional
set -e
...
# Install Node.js 22.x LTS (needed to build the React app)
...
for i in $(seq 1 60); do
  ...
  if [ -n "$${MY_IP}" ] && [ "$${RESOLVED}" = "$${MY_IP}" ]; then
    echo "  DNS resolved — checking certificate status"
    break
  elif [ -z "$${MY_IP}" ] && [ -n "$${RESOLVED}" ]; then
    echo "  DNS resolved (IMDS unavailable, trusting Terraform A record) — checking certificate status"
    break
  fi
  sleep 30
done
...
# ----------------------------------------------------------------------------
# Check if a valid local certificate already exists with >= 10 days remaining.
# This module uses Nginx/Certbot locally (HTTP-01) — no ACM involved.
# Skipping certbot prevents hitting Let's Encrypt rate limits on redeployments.
# ----------------------------------------------------------------------------
SKIP_CERTBOT=false
CERT_FILE="/etc/letsencrypt/live/${domain_name}/fullchain.pem"
if [ -f "$${CERT_FILE}" ]; then
  EXPIRY_STR=$(openssl x509 -in "$${CERT_FILE}" -noout -enddate 2>/dev/null | cut -d= -f2)
  if [ -n "$${EXPIRY_STR}" ]; then
    EXPIRY_EPOCH=$(date -d "$${EXPIRY_STR}" +%s 2>/dev/null)
    NOW_EPOCH=$(date +%s)
    DAYS_REMAINING=$(( (EXPIRY_EPOCH - NOW_EPOCH) / 86400 ))
    echo "  Existing local cert expires in $${DAYS_REMAINING} days ($${EXPIRY_STR})"
    if [ "$${DAYS_REMAINING}" -ge 10 ]; then
      echo "  Certificate has $${DAYS_REMAINING} days remaining (>= 10) — skipping certbot request."
      SKIP_CERTBOT=true
    else
      echo "  Certificate expires in $${DAYS_REMAINING} days (< 10) — requesting renewal."
    fi
  fi
else
  echo "  No existing local certificate found — requesting new certificate."
fi
